function [opt_list] = plot_optical_constants(superstrate, stack, substrate, wavelengths, do_plot)
...
for i1 = 1:length(stack)
    for j1 = 1:length(stack(i1).layer)
        if ~any(strcmp(filenames,stack(i1).layer(j1).mat{1}))
            ...
            filenames(end+1) = stack(i1).layer(j1).mat(1);
        end
    end
end
...
if do_plot
    ...
    for i1 = 1:length(opt_list)
        yyaxis left
        plot(wavelengths/1000, real(opt_list(i1).material.refractive_index), '-', 'DisplayName', opt_list(i1).material.filename, 'Color', cols(i1,:), 'LineWidth', 1)
        yyaxis right
        h = plot(wavelengths/1000, imag(opt_list(i1).material.refractive_index), '--', 'Color', cols(i1,:), 'LineWidth', 1);
