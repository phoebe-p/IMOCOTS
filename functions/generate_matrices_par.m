function [] = generate_matrices_par(superstrate, stack, substrate, options)
...
parfor i1 = 1:length(lambda)
    disp(['Wavelength ' num2str(i1) ' of ', num2str(length(lambda)), ':'])
    if exist(strcat(options.name, '/A_', stack(1).name, num2str(lambda(i1)), 'nm_sum.mat'), 'file') == 0
    ...
    if exist(strcat(options.name, '/C_', stack(3).name, num2str(lambda(i1)), 'nm_sum.mat'), 'file') == 0
