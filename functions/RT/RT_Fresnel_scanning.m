function [Rmat, Tmat, Amat, angle_vector, theta_summary] = RT_Fresnel_scanning(incidence, behind, texture, options, wavelength)
...
n_y = options.n_scans;
n_tries = 5;
n_ray_angles = ceil(sqrt(options.n_rays/(n_x*n_y*n_tries)));

% get optical constants
nk=load(incidence);
n1=real(interp1(nk(:,1)*1000,nk(:,2),wavelength)+1i*interp1(nk(:,1)*1000,nk(:,3),wavelength));
nk=load(behind);
n2=real(interp1(nk(:,1)*1000,nk(:,2),wavelength)+1i*interp1(nk(:,1)*1000,nk(:,3),wavelength));
...
theta_in = repmat(linspace(0.01, theta_max, n_angles), 1, n_tries);
phi_in = linspace(0.01, phi_max, n_angles);
[theta_in, phi_in] = meshgrid(theta_in, phi_in);
length(theta_in(:))
%poolobj = gcp;
...
if usemean == true
    x_in = linspace(Lx/100+(Lx/4), Lx/2, n_x);
    y_in = linspace(Ly/100+(Ly/4), Ly/2, n_y);
    offset = mean(tr.Points(:,3));
else
    x_in = linspace(Lx/100, Lx/2, n_x);
    y_in = linspace(Ly/100, Ly/2, n_y);
    offset = 0;
end
