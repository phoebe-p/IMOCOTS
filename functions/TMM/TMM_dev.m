...
absorb_inc_tr = options.incident_absorption;
...
for i1 = 1:(length(layers))
    stack(i1).material_filename=layers(i1).mat{1};
    stack(i1).refractive_index=[];
    stack(i1).thickness=layers(i1).thickness;          %nm
end
...
for j1= 1:length(angle_vector)/2
 ...
 %       phi_in = angle_vector(phi_ind, 3)+0.1;
        ...
        theta_summary(theta_o_bin_R, theta_i_bin) = theta_summary(theta_o_bin_R, theta_i_bin) + R;
        ...
        theta_summary(theta_o_bin_T, theta_i_bin) = theta_summary(theta_o_bin_T, theta_i_bin) + T;
        
        % Absorption
        matrix_res(end, in_loc) = matrix_res(end, in_loc) + A;
        theta_summary(end, theta_i_bin) = theta_summary(theta_o_bin_T, theta_i_bin) + A;
